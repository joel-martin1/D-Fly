...
import java.time.temporal.ChronoUnit; // Import necesario para calcular dias
	...
	public VentanaPrincipal() {
		...
		chkSoloIda.addActionListener(e -> {
			...
			if (soloIda) {
				...
				rbAlojVuelo.setEnabled(false);
			} else {
				rbAlojamiento.setEnabled(true);
				rbAlojVuelo.setEnabled(true);
			}
		});
		...
		btnBuscar.addActionListener(e -> {
			...
			if (!soloHotel && ciudadOrigen.isEmpty()) {
				JOptionPane.showMessageDialog(this, "Para buscar vuelos necesitas un origen", "Aviso", JOptionPane.WARNING_MESSAGE);
				return;
			}
			
			if (!soloHotel && ciudadOrigen.equalsIgnoreCase(ciudadDestino)) {
				JOptionPane.showMessageDialog(this, "El origen y destino no pueden ser iguales", "Error", JOptionPane.WARNING_MESSAGE);
				return;
			}
			...
			if (!chkSoloIda.isSelected()) {
				...
				int mIda = cbmMesIda.getSelectedIndex() + 1;
				int aIda = (Integer) cbmAnioIda.getSelectedItem();
				LocalDate fIda = LocalDate.of(aIda, mIda, dIda);

				int dVuelta = (Integer) cbmDiaVuelta.getSelectedItem();
				int mVuelta = cbmMesVuelta.getSelectedIndex() + 1;
				int aVuelta = (Integer) cbmAnioVuelta.getSelectedItem();
				LocalDate fVuelta = LocalDate.of(aVuelta, mVuelta, dVuelta);

				long diasCalc = ChronoUnit.DAYS.between(fIda, fVuelta);
				if (diasCalc < 1) {
					JOptionPane.showMessageDialog(this, "La fecha de vuelta debe ser posterior a la ida", "Error", JOptionPane.WARNING_MESSAGE);
					return;
				}
				dias = (int) diasCalc;
			}
			...
			Destino origenObj = DBManager.getDestinoById(idOrigen);
			
			int dIda = (Integer) cbmDiaIda.getSelectedItem();
			int mIda = cbmMesIda.getSelectedIndex() + 1;
			int aIda = (Integer) cbmAnioIda.getSelectedItem();
			String fecha = String.format("%04d-%02d-%02d", aIda, mIda, dIda);
